...
public class MatrixDriver {
	public static void main(String[] args) {
		...
		do {
			...
			case ("P"):
				...
				break;
			case ("AR"):
				Matrix arMat = m1.add(m1.reverseRows());
				arMat.printMatrix();
				break;
			case ("AN"):
				System.out.print("Please add an integer we will add to every spot on the matrix");
	...
	public static void printMenu() {
		...
		System.out.println("  P PrintMatrix - This will print the original matrix");
		System.out.println("  AR AddReverse - Add the reverse to your Original Matrix");
		System.out.println("  AN AddNumber  - Add a Number to each value in the Matrix");
...
class Matrix {
	...
	// Constructor that initializes the matrix to 0 1 2, then 10 11 12, then 20 21
	// 22, ...
	...
	Matrix add(Matrix m2) {
		...
		Matrix madd = new Matrix(mat.length, mat[0].length);
		for (int i = 0; i < mat.length; i++) {
			for (int j = 0; j < mat[i].length; j++) {
				madd.mat[i][j] = mat[i][j] + m2.mat[i][j];
			}
		}
		return madd;
	}
